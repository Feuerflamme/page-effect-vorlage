...
// Error handling for block
// error_reporting(E_ALL);
// ini_set('display_errors', 1);

// Debug: Log block execution
// if (defined('WP_DEBUG') && WP_DEBUG) {
//     error_log('Hero block rendering started');
// }
  ...
  <div class="container has-columns">
    ...
    <div class="column col-text">
      <div class="text-wrapper">
        <?php 
        $headline = get_field('seitenuberschrift_h1'); 
        ...
        <?php if ($button) : ?>
          <div class="button-wrapper" role="group" aria-label="Actions">
            ...
            <span id="<?php echo esc_attr($button['title']); ?>" class="sr-only">(Description for screen readers)</span>
          </div>
        <?php endif; ?>
      </div>
    </div>
    ...
    <div class="column col-image">
      ...
      if (get_field('slider') == 0) :
        ...
        if ($image): ?>
          <!-- Responsive image with proper srcset -->
          <img
            ...
            width="<?= $image['width'] ?>"
            height="<?= $image['height'] ?>"
            onload="console.log('Hero Image loaded:', this.currentSrc || this.src)">
        <?php endif; ?>
